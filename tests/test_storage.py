...
def test_in_memory_jsonl_writer_uses_pydantic_compatible_serialization() -> None:
    store = InMemoryRawStore()
    items = [TextItem(source_name="rss", source_item_id="1", text="hello")]

    key = store.write_items_jsonl(items, prefix="source=rss")

    assert key == "source=rss/batch.jsonl"
    assert list(store.iter_items(prefix="source=rss"))[0] == items[0]
