...
## 7. Запуск MinIO локально

Запустите MinIO и тестовый HTTP endpoint из `docker-compose.yml`:
...
MinIO console будет доступна по адресу:
...
Создайте bucket из настроек `.env`:
