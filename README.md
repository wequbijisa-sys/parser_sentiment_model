...
├── Dockerfile
├── docker-compose.yml             # Local MinIO and placeholder echo endpoint
└── pyproject.toml
```
...
### Local MinIO and placeholder endpoint

```bash
docker compose up -d minio http-echo
python scripts/bootstrap_minio.py
```

MinIO console: <http://localhost:9001>

Default credentials in local compose are `minioadmin` / `minioadmin`.
